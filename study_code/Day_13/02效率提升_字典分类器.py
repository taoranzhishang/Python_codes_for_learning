...
fileDict = {}  # 字典
for key in myDict:
    filepath = "D:\\code\\py\\study_code\\Day_12\\开房数据清洗\\area\\" + myDict[key] + ".txt"
    infoFile = open(filepath, "wb")
    fileDict[myDict[key]] = infoFile  # 插入字典
...
for line in myList:  # 遍历结果为键值对组成的tuple
    ...
    if areaCode in myDict.keys():
        if myDict[areaCode] in fileDict.keys():
            fileDict[myDict[areaCode]].write(line.encode("utf-8"))
print("over")
for key in fileDict:  # 遍历文件对象进行关闭
    fileDict[key].close()
